...
public class SearchRemoteDataSource: DataSource {
    public typealias Request = String
    public typealias Response = SearchNewsResponse

    public init() {}

    public func executeObservable(request: Request) -> Observable<Response> {
        return Observable.create { observer in
            let from = Endpoints.everything.url
            let params = NetworkConstants.constructParams(query: request)
